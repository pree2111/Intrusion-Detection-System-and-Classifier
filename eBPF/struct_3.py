...
# header
print("%-8s %-16s %-16s %-8s %-8s %-8s %-20s %-10s" % ("TYPE" , "SADDR", "DADDR", "PROTOCOL", "I-COUNT", "O-COUNT", "TIMESTAMP", "BYTES"))
...
while True:
    ...
    printb(b"%-8s %-16s %-16s %-8s %-8d %-8d %-20s %-10d" % (_type, saddr.encode(), daddr.encode(), protocol_name.encode(), info.incoming_pkt_count, info.outgoing_pkt_count, timestamp.encode(), info.bytes))
