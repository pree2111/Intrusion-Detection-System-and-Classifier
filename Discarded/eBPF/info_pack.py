...
int kprobe__tcp_rcv_established(struct pt_regs *ctx, struct sock *sk, struct sk_buff *skb)
{
    ...
    struct iphdr *ip_header = (struct iphdr *)(skb->data);
    struct tcphdr *tcp_header = (struct tcphdr *)(skb->data + sizeof(struct iphdr));
    u16 tcp_flags;
    ...
    struct info *info = packet_info.lookup_or_init(&pass_value, &pkt_info);
    // Increment the incoming packet count
    info->incoming_pkt_count++;

    bpf_trace_printk("IN TCP %x %x\\n", pkt_info.saddr, pkt_info.daddr);

    return 0;
}

int kprobe__tcp_v4_send_check(struct pt_regs *ctx, struct sock *sk, struct sk_buff *skb)
{
    struct info pkt_info = {};
    u64 pass_value = 0;
    ...
    struct info *info = packet_info.lookup_or_init(&pass_value, &pkt_info);
    // Increment the outgoing packet count
    info->outgoing_pkt_count++;

    bpf_trace_printk("OUT TCP %x %x\\n", pkt_info.saddr, pkt_info.daddr);
...
# Header for CSV file
header = ["TYPE", "SADDR", "DADDR", "PROTOCOL", "I-COUNT", "O-COUNT", "TIMESTAMP", "BYTES", "FLAGS"]
...
with open('packet_info.csv', 'w', newline='') as csvfile:
    ...
    while True:
        ...
        flags = []
        if info.protocol == 6:  # TCP protocol
            if info.flags & 1:
                flags.append("SYN")
            if info.flags & 2:
                flags.append("ACK")
        ...
        row = [_type.decode(), saddr, daddr, protocol_name, info.incoming_pkt_count, info.outgoing_pkt_count, timestamp, info.bytes, "|".join(flags)]
        writer.writerow(row)
